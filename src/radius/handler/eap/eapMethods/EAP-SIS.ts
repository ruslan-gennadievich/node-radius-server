...
export class EAPSIS implements IEAPMethod {
	...
	async handleMessage(
		...
		const server_challenge = _msg.slice(offset, offset + len + 1);

		[offset, len] = [offset + len, 16];
		const peer_challenge = _msg.slice(offset, offset + len + 1);

		// Sign Encryptor
		[offset, len] = [offset + len, 2];
		const ecr_sign_offset = _msg.slice(offset, offset + len + 1).readUInt16LE(0);

		[offset, len] = [offset + len, 2];
		const ecr_sign_size = _msg.slice(offset, offset + len + 1).readUInt16LE(0);

		// Sign User
		[offset, len] = [offset + len, 2];
		const user_sign_offset = _msg.slice(offset, offset + len + 1).readUInt16LE(0);

		[offset, len] = [offset + len, 2];
		const user_sign_size = _msg.slice(offset, offset + len + 1).readUInt16LE(0);

		// Cert Encryptor
		[offset, len] = [offset + len, 2];
		const ecr_cert_offset = _msg.slice(offset, offset + len + 1).readUInt16LE(0);
		...
		// Cert User
		[offset, len] = [offset + len, 2];
		const user_cert_offset = _msg.slice(offset, offset + len + 1).readUInt16LE(0);

		[offset, len] = [offset + len, 2];
		const user_cert_size = _msg.slice(offset, offset + len + 1).readUInt16LE(0);
